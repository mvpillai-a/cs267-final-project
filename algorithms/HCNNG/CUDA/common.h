// Enhanced common.h with improved data structures and algorithms
...
// Enhanced DisjointSet with path compression for better performance
struct DisjointSet {
   int* parent;
   int* rank;
   ...
   int find(int x) {
       if(parent[x] != x)
           parent[x] = find(parent[x]); // Path compression for better performance
       return parent[x];
   }
...
// Edge structure with enhanced comparison
struct Edge {
   int v1, v2;
   float weight;
   ...
   Edge(int _v1, int _v2, float _weight) : v1(_v1), v2(_v2), weight(_weight) {}
   ...
   bool operator==(const Edge& e) const {
       return v2 == e.v2; // For removing duplicates in vectors
   }
...
struct Matrix {
   ...
   Matrix() : rows(-1), cols(-1), M(nullptr) {}
...
// Optimized edge duplicate removal
void remove_edge_duplicates(int v, Graph &G) {
   if (G[v].size() <= 1) return;
  
   // Sort edges by destination
   sort(G[v].begin(), G[v].end(), [](const Edge& a, const Edge& b) {
       return a.v2 < b.v2;
   });
  
   // Remove duplicates
   auto new_end = unique(G[v].begin(), G[v].end(), [](const Edge& a, const Edge& b) {
       return a.v2 == b.v2;
   });
  
   G[v].erase(new_end, G[v].end());
}


// Remove all duplicates in the graph
void remove_all_duplicates(Graph &G) {
   #pragma omp parallel for
...
// Add an edge with duplicate checking
void add_edge(int u, int v, float weight, Graph &G, int max_degree) {
   // Check for duplicates
   for (const Edge& e : G[u]) {
       if (e.v2 == v) return;
   }
  
   // Add if under max degree, or replace worst edge
   if (G[u].size() < max_degree) {
       G[u].push_back(Edge(u, v, weight));
   } else {
       // Find worst edge
       auto max_it = max_element(G[u].begin(), G[u].end(),
           [](const Edge& a, const Edge& b) { return a.weight < b.weight; });
      
       if (max_it->weight > weight) {
           *max_it = Edge(u, v, weight);
       }
   }
}
...
// Enhanced MST creation function with priority queue approach
...
// Select two random points optimally
pair<int, int> select_two_random(int left, int right) {
   int range = right - left + 1;
   int x = left + rand() % range;
   int y;
